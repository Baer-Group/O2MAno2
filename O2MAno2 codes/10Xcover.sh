...
#SBATCH --job-name=E10Xhcover # Job name
...
#SBATCH --output=E10Xhcover_%j.out # Standard output and error log
...
input_dir="/orange/baer/E_new/H7_file_pipeline"



# Output CSV file in the current working directory

output_file="E10Xh_coverage_results.csv"
...
find "$input_dir" -type f -name "*.markduplicates.bam" | while read -r bam_file; do
  ...
  samtools depth -a "$bam_file" > h_temp.txt



  # Calculate total covered length (number of bases with coverage >= 3)

  total_covered_length=$(awk '{ if ($3 >= 10) covered += 1 } END { print covered }' h_temp.txt)



  # Calculate total reads and average depth

  total_reads=$(awk '{sum += $3} END {print sum}' h_temp.txt)
