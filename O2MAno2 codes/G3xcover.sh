...
#SBATCH --job-name=G10xhcover # Job name
...
#SBATCH --output=G10xhcover_%j.out # Standard output and error log
...
output_file="G10xh_coverage_results.csv"
...
find "$input_dir" -type f -name "*.markduplicates.bam" | while read -r bam_file; do
  ...
  samtools depth -a "$bam_file" > coverage_temp.txt



  # Calculate total covered length (number of bases with coverage >= 3)

  total_covered_length=$(awk '{ if ($3 >= 10) covered += 1 } END { print covered }' coverage_temp.txt)



  # Calculate total reads and average depth

  total_reads=$(awk '{sum += $3} END {print sum}' coverage_temp.txt)
